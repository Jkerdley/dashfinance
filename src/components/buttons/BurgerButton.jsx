...
export const BurgerButton = ({ isOpen, onClick }) => {
	return (
		<button
			onClick={onClick}
			className="group flex md:flex sm:flex 2xl:hidden xl:flex flex-col justify-center items-center lg:w-10 lg:h-10 md:w-9 md:h-9 w-8 h-8 rounded-lg outline-1
		...
		>
          ...
          `}
			...
			<span
				className={`
            block w-5 h-0.5 bg-gray-50 rounded-lg transition-all duration-300 ease-in-out my-1.5
            group-hover:bg-blue-400
            ${isOpen ? 'opacity-0' : ''}
          `}
			/>
