...
export const FinanceResult = memo((props: FinanceResultProps) => {
    ...
    const totalBalanceForDate = useMemo(() => {
        ...
        return calculateValueInCurrency(accountsDB, isUSD, rubleCourse || 1);
    ...
    const incomeForDate = useMemo(() => {
        ...
        return calculateValueInCurrency(historyDB, isUSD, rubleCourse || 1);
    }, [financeHistory, isUSD, rubleCourse]);

    const expensesForDate = useMemo(() => {
        const categoriesDB = expenses.mappedData.reduce((acc, item: any) => acc + (item.balance || 0), 0);
        return calculateValueInCurrency(categoriesDB, isUSD, rubleCourse || 1);
    ...
    return (
        <div id="finance-result__main-container" className="flex h-full gap-4 transition-all">
            <div className="flex flex-col flex-3 h-full">
                ...
                <section className="flex flex-2 h-full justify-center md:justify-evenly gap-6 md:gap-18">
                    {isLoading ? (
                        <Loader />
                    ) : (
                        <IncomeResult
