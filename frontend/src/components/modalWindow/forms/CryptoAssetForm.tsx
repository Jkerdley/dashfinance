...
interface CryptoAssetFormProps {
    ...
    error?: string;
    onClose: () => void;
    handleFindCrypto: (e: ChangeEvent<HTMLInputElement>) => void;
    handleSelectCoin: (coin: CoinData) => void;
    searchResults: CoinData[];
                        ...
                        <div className="absolute top-full left-0 right-0 z-50 bg-sky-950/90 max-h-70 overflow-y-auto mt-1 rounded-lg scrollbar">
                            {searchResults.map((coin) => (
                                ...
                                >
                                    ...
                                    {coin.name}
                                    <span className="text-slate-400 text-sm ml-1">{coin.symbol}</span>
                                </div>
                            ))}
                        </div>
                    )}
